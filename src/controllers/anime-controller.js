...
async function setNovoAnime(req, res) {
    const { nome, ano, nota, genero, episodios, imagem, sinopse } = req.body;

    await animesModel.setNovoAnimeModel(nome, ano, nota, genero, episodios, imagem, sinopse)
...
async function updateAnime(req, res) {
    ...
    await animesModel.updateAnimeModel(episodios, id);


    return res.send("Episodio atualizados com sucesso");
}
